                       ...
                       <div class="row-one">
                           ...
                           <div class="content-user">
                               <v-simple-table class="table-data-user">
                                   ...
                                   <tr>
                                       <td>
                                           <span class="font-card-user" v-if="$t('lang') === 'ru'">  {{user.name_ru}}</span>
                                           <span class="font-card-user" v-if="$t('lang') === 'en'">  {{user.name_en}} </span>
                                       </td>
                                   </tr>
                                   <tr>
                                       <td>
                                           <span class="font-card-user">{{$t("loginBlock.form.position")}}: </span>
                                           <span class="font-card-user" v-if="$t('lang') === 'ru'">  {{user.position_ru}} </span>
                                           <span class="font-card-user" v-if="$t('lang') === 'en'">  {{user.position_en}} </span>
                                       </td>
                                   </tr>
                                   <tr>
                                       <td>
                                           <span class="font-card-user">{{$t("loginBlock.form.section")}}: </span>
                                           <span class="font-card-user" v-if="$t('lang') === 'ru'">  {{user.section_ru}} </span>
                                           <span class="font-card-user" v-if="$t('lang') === 'en'">  {{user.section_en}} </span>
                                       </td>
                                   </tr>
                                   <tr>
                                       <td>
                                           <span class="font-card-user">{{$t("loginBlock.form.state")}}: </span>
                                           <span class="font-card-user" v-if="$t('lang') === 'ru'">  {{user.state_ru}} </span>
                                           <span class="font-card-user" v-if="$t('lang') === 'en'">  {{user.state_en}} </span>
                                       </td>
                                   </tr>
                                   <tr>
                                       <td>
                                           <span class="font-card-user">{{$t("loginBlock.form.city")}}: </span>
                                           <span class="font-card-user" v-if="$t('lang') === 'ru'">  {{user.city_ru}} </span>
                                           <span class="font-card-user" v-if="$t('lang') === 'en'">  {{user.city_en}} </span>
                                       </td>
                                   </tr>
                                   <tr>
                                       <td>
                                           <span class="font-card-user">{{$t("loginBlock.form.nomination")}}: </span>
                                           <span class="font-card-user" v-if="$t('lang') === 'ru'">  {{user.nomination_ru}} </span>
                                           <span class="font-card-user" v-if="$t('lang') === 'en'">  {{user.nomination_en}} </span>
                                       </td>
                                   </tr>
                                   </tbody>
                               </v-simple-table>
                        ...
                        <div class="voting-selection">
                            ...
                            <v-btn
                                ...
                                :disabled="disabledVoting"
                            >{{$t("participantID.buttonVoting")}}</v-btn>
                            <p class="mt-3" v-if="$t('lang') === 'ru'">{{count_voting_ru}}</p>
                            <p class="mt-3" v-if="$t('lang') === 'en'">{{count_voting_en}}</p>
                        </div>
                    </v-card>
                </v-col>
            </v-row>
        ...
        methods: {
            ...
            parseUserData: function (data) {
                const newObject = {
                    ...
                    name_ru: data.lastname_ru + ' ' + data.firstname_ru + ' ' + data.patronymic_ru,
            ...
            getVotes: function (voting_id) {
                ...
                const url = config.API_URL + '/votes/from/' + jwtHelper.jwtParse().id + '/voting/' + voting_id;
                axios.get(
                    url,
                    ...
                    .then(result => {
                        ...
                        if( if_1 ) {
                            ...
                            this.count_voting_ru = 'Вы можете голосовать только за участника своего региона';
                            this.count_voting_en = 'You can only vote for a member of your region';
                            this.disabledVoting = true;
                        } else {

                            const userFind = result.data.find(item => +item.user_to_id === +this.$route.params.id);
                            if (userFind !== undefined) {
                                this.count_voting_ru = 'Вы уже голосовали за этого участника';
                                this.count_voting_en = 'You have already voted for this member';
                                this.disabledVoting = true;
                            } else {
                                if(jwtHelper.isCommittee()) { //Check committee
