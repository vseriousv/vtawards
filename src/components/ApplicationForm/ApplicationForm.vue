...
export default {
	...
	methods: {
		...
		setNomination: function(nominations) {
			(this.nominationSelect = ""), (this.nominationItemsRu = []);
		...
		postDataForm: function() {
			...
			(this.user = {}), (this.userValue = "");
			this.nominationSelect = "";
			this.argumentationText = "";
			(this.argumentationFile = []), (this.file = []);
			this.formsArr = [];
		},
		...
		PostFormNomination: async function(data) {
			...
			try {
				...
				alert(
					"Ваша заявка успешно сформирована и отправлена. Она будет опубликована после модерации администратором."
				);
			} catch (e) {
