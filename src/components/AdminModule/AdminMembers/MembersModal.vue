    ...
    export default {
        ...
        methods: {
            sendData: function () {
                ...
                if( this.$store.state.memberUser_Id === '' || this.$store.state.memberVotingId === ''){
            ...
            parserUserData: function (data) {
                ...
                for (let i = 0; i < data.length; i++){
                    const text_ru = data[i].tab_number + ': ' + data[i].firstname_ru + ' ' + data[i].lastname_ru + ' ' + data[i].patronymic_ru;
        ...
        computed: {
            memberId: {
                get () {  return this.$store.state.memberId;  },
                set (id) { this.$store.commit("setMemberId", id); }
            },
            memberUser_Id: {
                get () { return this.$store.state.memberUser_Id; },
                set (value) { this.$store.commit("setMemberUser_Id", value); }
            },
            memberVotingId: {
                get () { return this.$store.state.memberVotingId; },
                set (value) { this.$store.commit("setMemberVotingId", value); }
            },
            adminErrorStr: {
