        ...
        <v-simple-table class="tableAll">
            <template v-slot:default>
                ...
                <tr>
                    ...
                    <th class="text-center">{{$t("membersBlock.table.count_committee_votes")}}</th>
                    <th class="text-center">{{$t("membersBlock.table.committee_rating")}}</th>
                    <th class="text-center">{{$t("membersBlock.table.sr_rating")}}</th>
                    <th class="text-center">{{$t("membersBlock.table.count_votes")}}</th>
<!--                    <th class="text-center">{{$t("membersBlock.table.result_rating")}}</th>-->
                </tr>
                </thead>
                <tbody>
                <tr v-for="(itemField, id) in committee"  class="tr_row" :key="`committeeItem${id}`">
                    <td class="td_block text-center">
                        <div class="avatar">
                            <img :src="`${URL_AVATARS}${itemField.img}`" />
                        </div>
                    </td>
                    <td v-if="$t('lang') === 'ru'" class="td_block">{{ itemField.name_ru}}</td>
                    <td v-if="$t('lang') === 'en'" class="td_block">{{ itemField.name_en}} </td>
                    <td class="td_block text-center">{{ itemField.committee_rating}} </td>
                    <td class="td_block text-center">{{ itemField.count_committee_votes}} </td>
        ...
        methods: {
            ...
            setUsersArray: async function (data) {
                for (let i = 0; i < data.length; i++){
                    ...
                    let committee_avg = 0;
                    if (data[i].result) {
                        committee_avg = data[i].result.committee_votes !== 0 ? +data[i].result.committee_votes / +data[i].result.committee_rating : 0;
                    }
                    ...
                    const userObject = {
                        ...
                        count_committee_votes: data[i].result ? data[i].result.committee_rating: 0,
