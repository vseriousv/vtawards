<template>
  <section v-if="typeVoting != ''" class="winner-all-block">
    <h2 class="text-center">
      <span v-if="typeVoting === 'semifinal_voting'">{{$t("winnersAllBlock.head.text1")}} {{$t("winnersAllBlock.head.semifinal_voting") }}</span>
      <span v-if="typeVoting === 'autumn_committee_voting'">{{$t("winnersAllBlock.head.text1")}} {{$t("winnersAllBlock.head.autumn_committee_voting") }}</span>
      <span v-if="typeVoting === 'autumn_users_voting'">{{$t("winnersAllBlock.head.text1")}} {{$t("winnersAllBlock.head.autumn_users_voting") }}</span>
      <span v-if="typeVoting === 'spring_committee_voting'">{{$t("winnersAllBlock.head.text1")}} {{$t("winnersAllBlock.head.spring_committee_voting") }}</span>
      <span v-if="typeVoting === 'spring_users_voting'">{{$t("winnersAllBlock.head.text1")}} {{$t("winnersAllBlock.head.spring_users_voting") }}</span>
    </h2>
    <v-container>
      <v-row>
        <v-col cols="12" class="d-flex winnerCards">
          <div class="winnerCard" v-for="(winner, id) in winners" :key="`winner${id}`">
            <v-avatar size="100px">
              <img
      ...
      getWinner: function (id) {
        ...
        axios.get(url, {headers: {Authorization: "Bearer " + localStorage.getItem("jwt")}})
          .then(result => {
            ...
            this.yearVoting = result.data.length === 0 ? 0 : result.data[0].voting.year;
            this.typeVoting = result.data.length === 0 ? 0 : result.data[0].voting.type_voting;
      ...
      setUsersArray: async function (data) {
        for (let i = 0; i < data.length; i++){
          const userObject = {
            ...
            name_ru: data[i].user.lastname_ru + '<br />' + data[i].user.firstname_ru + ' ' + data[i].user.patronymic_ru,
