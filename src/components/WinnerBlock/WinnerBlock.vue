		...
		>
			<v-row>
				...
				>
					...
					<div>
						<span v-if="$t('lang') === 'ru'"
							>Должность: {{ winner.position_ru }}</span
						>
						<span v-if="$t('lang') === 'en'"
							>Position: {{ winner.position_en }}</span
						>
					</div>
					<div>
						<span v-if="$t('lang') === 'ru'"
							>Регион: {{ winner.state_ru }}</span
						>
						<span v-if="$t('lang') === 'en'"
							>Region: {{ winner.state_en }}</span
						>
					</div>
					<div>
					...
					<div>
						<span v-if="$t('lang') === 'ru'"
							>Номинация: {{ winner.nomination_ru }}</span
						>
						<span v-if="$t('lang') === 'en'"
							>Nomination: {{ winner.nomination_en }}</span
						>
					</div>
					<div class="mt-6">
						<v-btn color="primary" :to="`/participants/id/${winner.id}`">
							{{ $t("winnerBlock.more") }}
						</v-btn>
					</div>
				</v-col>
